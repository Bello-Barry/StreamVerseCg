/**
 * Service amélioré pour la gestion des torrents côté client
 */
...
// Interface pour le client WebTorrent global
interface WebTorrentClient {
  add: (torrentId: string | File | ArrayBuffer, opts?: any) => any;
  destroy: () => void;
  torrents: any[];
}

// Client WebTorrent global pour réutilisation
let globalWebTorrentClient: WebTorrentClient | null = null;

/**
 * Initialise le client WebTorrent global
 */
async function getWebTorrentClient(): Promise<WebTorrentClient> {
 ...
 */
export function extractNameFromMagnet(magnetURI: string): string | null {
 ...
 */
export async function parseTorrentContentImproved(
  source: string | File,
  sourceName: string
): Promise<TorrentParserResult> {
  console.log(`[parseTorrentContentImproved] Parsing torrent for source: ${sourceName}`);
  ...
  try {
    ...
    // Préparer la source pour WebTorrent
    let torrentToAdd: string | ArrayBuffer;
    ...
    const groupedContent = groupVideoFilesImproved(videoFiles, sourceName, torrentData);
    ...
    groupedContent.movies.forEach((movieData, index) => {
      const movie: Movie = {
        ...
        category: detectCategoryImproved(movieData.name),
        ...
        quality: detectQuality(movieData.name),
        year: detectYear(movieData.name)
      };
      movies.push(movie);
    });
    ...
    groupedContent.series.forEach((seriesData, index) => {
      const episodes: Episode[] = seriesData.episodes.map((ep, epIndex) => ({
        ...
        magnetURI: torrentData.magnetURI,
        torrentFile: ep.file,
        quality: detectQuality(ep.file.name),
        duration: estimateDuration(ep.file.length)
      }));
      ...
      const seriesItem: Series = {
        ...
        category: detectCategoryImproved(seriesData.name),
        playlistSource: sourceName,
        episodes,
        // Métadonnées supplémentaires
        totalSeasons: Math.max(...episodes.map(ep => ep.season)),
        year: detectYear(seriesData.name)
      };
 ...
 */
function groupVideoFilesImproved(files: any[], sourceName: string, torrentData: any) {
  const movies: any[] = [];
  const series: any[] = [];
  ...
  files.forEach((file: any) => {
    ...
    const episodeMatch = detectEpisodeImproved(fileName);
    ...
    if (episodeMatch) {
      ...
      seriesMap.get(seriesName)!.episodes.push({
        name: formatEpisodeName(episodeMatch),
      ...
      // C'est un film
      movies.push({
        name: cleanMovieNameImproved(fileName),
        files: [file]
      });
    }
  });
 ...
 */
function detectEpisodeImproved(fileName: string) {
  const patterns = [
    // Patterns standards
    /(.+?)[.\s]S(\d{1,2})E(\d{1,2})/i,
    /(.+?)[.\s](\d{1,2})x(\d{1,2})/i,
  ...
  for (const pattern of patterns) {
    ...
    if (match) {
      return {
        seriesName: cleanSeriesNameImproved(match[1]),
 ...
 */
function cleanSeriesNameImproved(name: string): string {
  return name
    .replace(/[._\-]/g, ' ')
    .replace(/\s+/g, ' ')
 ...
 */
function cleanMovieNameImproved(fileName: string): string {
 ...
 */
function detectCategoryImproved(name: string): string {
 ...
 */
function formatEpisodeName(episodeMatch: any): string {
 ...
 */
export function cleanupWebTorrentClient(): void {
  if (globalWebTorrentClient) {
    try {
      globalWebTorrentClient.destroy();
      globalWebTorrentClient = null;
    } catch (error) {
      console.error('Erreur lors du nettoyage WebTorrent:', error);
    }
  }
}