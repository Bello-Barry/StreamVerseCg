...
class SmartChannelRecommendation {
  ...
  getSmartRecommendations(allChannels: Channel[], options: SmartRecommendationOptions = {}): Channel[] {
    const {
    ...
    let filteredChannels = allChannels.filter(channel => {
      ...
      const status = channelValidator.getChannelStatus(channel.id)
      if (status && status.reliability < minReliability) return false
  ...
  getReliableChannelsByCategory(allChannels: Channel[], category: string, limit = 10): Channel[] {
  ...
  getChannelAlternatives(failedChannel: Channel, allChannels: Channel[], limit = 5): Channel[] {
    const sameCategory = allChannels.filter(channel =>
      channel.id !== failedChannel.id &&
      channel.category === failedChannel.category
    )

    const similarNames = allChannels.filter(channel =>
      channel.id !== failedChannel.id &&
  ...
  private calculateRecommendationScore(channel: Channel, options: {
    preferredCategories?: string[]
    boostPopularChannels?: boolean
  }): number {
    let score = 0
    ...
    if (status) {
      ...
      if (status.responseTime && status.responseTime < 2000) score += 10
    }
    ...
    if (options.preferredCategories?.includes(channel.category)) score += 15

    const recentIndex = this.lastWatchedChannels.indexOf(channel.id)
    if (recentIndex >= 3 && recentIndex <= 7) score += 10
...
const smartRecommendation = new SmartChannelRecommendation()
...
// ✅ Exports directs pour fichiers externes (Zustand, services, etc.)
export const getSmartRecommendations = smartRecommendation.getSmartRecommendations.bind(smartRecommendation)
export const getReliableChannelsByCategory = smartRecommendation.getReliableChannelsByCategory.bind(smartRecommendation)
export const getChannelAlternatives = smartRecommendation.getChannelAlternatives.bind(smartRecommendation)
export const updateWatchHistory = smartRecommendation.updateWatchHistory.bind(smartRecommendation)
export const getPopularChannels = smartRecommendation.getPopularChannels.bind(smartRecommendation)
export const getPreferredCategories = smartRecommendation.getPreferredCategories.bind(smartRecommendation)